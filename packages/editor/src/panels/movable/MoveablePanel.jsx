...
import { Button, Input, Typography, Icon } from '@douyinfe/semi-ui'
import { createMoveable } from '../../workspace/moveable'
import { IconClose, IconFullScreenStroked, IconHandle, IconShrinkScreenStroked } from '@douyinfe/semi-icons'
...
export default class MoveablePanel extends React.Component {
  ...
  render () {
    ...
    const { state, openSearch, search, minimize } = this.state
    ...
    if (Array.isArray(position) && position.length === 6) {
      ...
      if (left) {
        style.left = left + 'px'
        hiddenStyle.left = left + 'px'
      }
      if (top) {
        style.top = top + 'px'
        hiddenStyle.top = top + 'px'
      }
      if (right) {
        style.right = right + 'px'
        hiddenStyle.right = right + 'px'
      }
      if (bottom) {
    ...
    if (minimize) {
      panelClass.push('is-minimized')
    }

    return (
      ReactDOM.createPortal(
        <div ref={this.ref} className={panelClass.join(' ')} style={minimize ? hiddenStyle : style} id={this.props.id}>
          <div className='title-bar' style={titleStyle}>
            ...
            <Button
              ...
              }} className='icon-mininize' theme='borderless' type='tertiary' size='small' icon={minimize ? <IconFullScreenStroked /> : <IconShrinkScreenStroked />}
            />
