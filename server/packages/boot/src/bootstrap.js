...
const debug = require('debug')('ridge:boot')
const defaultConfig = require('./config')

function generateRandomId (length) {
  return crypto.randomBytes(Math.ceil(length / 2))
    .toString('hex') // 将随机字节转换为十六进制字符串
    .slice(0, length) // 截取到指定的长度
}
...
class BootStrap {
  ...
  async start () {
    ...
    process.on('exit', (code, e) => {
      debug('Process Exit  code = ', code, e)
    })

    process.on('SIGINT', code => {
      debug('Process Exit on SIGINT ', code)
  ...
  async processExit () {
    ...
    for (let i = 0; i < this.app.packages.length; i++) {
      ...
      try {
        packageModule.shutdown && await packageModule.shutdown(this.app)
      } catch (err) {
        // 关机执行出错就忽略了
