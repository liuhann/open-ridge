...
class NeCollection {
  ...
  async bulkInsert (objects) {
    debug('insert', objects)
    return new Promise((resolve, reject) => {
      this.store.insert(objects, function (err, newDoc) {
        if (err) {
          reject(err)
        } else {
          resolve(newDoc)
  ...
  getArchivePath () {
    const t = new Date()
    const timeFilePath = `fileStore/${t.getFullYear()}/${t.getMonth() + 1}/${t.getDate()}/${t.getHours()}/${t.getMinutes()}/${shortid('10')}`
  ...
  /**
     * 写入一个归档性质的文件
     * @param {Object} object 用于归档查询的对象，可以查询到对应文件
     * @param {Object} bigObject 归档对象，这个对象可以比较大，是写入磁盘的获取时需要额外调用getArchiveObject才能拿到
     * @returns 归档id
     */
  async insertArchive (object, bigObject) {
    const fsExtra = require('fs-extra')
    const archivePath = this.getArchivePath()
    const fullPath = path.resolve(this.filename, '../' + archivePath)

    await fsExtra.ensureDirSync(path.resolve(fullPath, '../'))
    if (typeof bigObject === 'object') { // json对象
      fsExtra.writeJSONSync(fullPath, bigObject)
    } else if (typeof bigObject === 'string') { // 文件（给路径） 或大的string串
      if (bigObject.length < 2000 && fsExtra.existsSync(bigObject)) {
        fsExtra.copyFileSync(bigObject, fullPath)
      } else {
        fsExtra.writeFileSync(fullPath, bigObject)
      }
    }

    object.__filePath = archivePath
    const insertResult = await this.insert(object)
  ...
  /**
     * 读取归档完整JSON文件
     * @param {string} id 归档ID
     * @returns JSON对象
     */
  async getArchiveObject (id) {
    const fsExtra = require('fs-extra')
    const object = await this.findOne(id)

    if (object.__filePath && fsExtra.existsSync(path.resolve(this.filename, '../' + object.__filePath))) {
      return fsExtra.readJSONSync(path.resolve(this.filename, '../' + object.__filePath))
    } else {
      return null
    }
  }
